...
export class SubCategoriesBotListService implements OnModuleInit {
    ...
    async getAllSubCategoriesList(bot: any, chatId: string, forSale: boolean): Promise<void> {
        try {
            ...
            if (subCategories.length > 0) {
                ...
                if (forSale) {
                    ...
                    await bot.sendMessage(
                        chatId,
                        'Выберите подкатегорию для продажи',
                        {
                            reply_markup: JSON.stringify({
                                inline_keyboard: cats
                            })
    ...
    async getSubCategoriesByCategoryId(bot: any, chatId: string, categoryId: number, forSale: boolean): Promise<void> {
        try {
            const subCategories: SubCategoryEntity[] = await this.subCategoriesService.getByCategory(categoryId);
    ...
    async getAllSubCategoriesForUpdateList(bot: any, chatId: string): Promise<void> {
    ...
    async getAllSubCategoryForDeleteList(bot: any, chatId: string): Promise<void> {
