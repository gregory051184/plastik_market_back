...
export class SubCategoryService {
  ...
  async create(subCategoryDto: SubCategoryDto): Promise<SubCategoryEntity> {
    ...
    const subCategory: SubCategoryEntity =
      await this.subCategoriesRepository.create(subCategoryDto);
  ...
  async getAll(): Promise<SubCategoryEntity[]> {
    const categories: SubCategoryEntity[] =
      await this.subCategoriesRepository.find();
    return categories;
  }

  async getById(id: number): Promise<SubCategoryEntity> {
    const category: SubCategoryEntity =
      await this.subCategoriesRepository.findOne({ where: { id: id } });
    return category;
  }

  async update(subCategoryUpdateDto: SubCategoryUpdateDto): Promise<void> {
    await this.subCategoriesRepository.update(subCategoryUpdateDto.id, {
