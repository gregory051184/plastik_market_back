...
import {citiesAdminMenuKeyboard, citiesUserMenuKeyboard, CityEntity, urls} from "@app/common";
...
export class CitiesBotListService implements OnModuleInit {
    ...
    async getAllCitiesList(bot: any, chatId: string, forSale: boolean): Promise<void> {
        try {
            ...
            if (cities.length > 0) {
                if (!forSale) {
                    const cit = await cities.map((city: any) =>
                        [{text: `${city.title}`, callback_data: `city_for_buying_${city.id}`}]
                    )
                ...
                if (forSale) {
                    const cit = await cities.map((city: any) =>
                        [{text: `${city.title}`, callback_data: `city_for_sale_${city.id}`}]
                    )
                    await bot.sendMessage(
                        chatId,
                        'Выберите город (продажа)',
                        {
                            reply_markup: JSON.stringify({
                                inline_keyboard: cit
                            })
    ...
    async getCityByTitleList(bot: any, chatId: string, forSale: boolean): Promise<void> {
        try {
            const currentUser: any = await this.usersService.getByChatId(chatId.toString());
            await bot.sendMessage(chatId, 'Введите название города').then(async () => {
                await bot.once('message', async (message: any) => {
                    const cities: any = await this.citiesService.getByTitle(this.commonService.messageText(message));
                    if (cities.length > 0) {
                        if (!forSale) {
                            const cit = await cities.map((city: any) =>
                                [{text: `${city.title}`, callback_data: `city_for_buying_${city.id}`}]
