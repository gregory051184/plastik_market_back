...
export class SubscribeService {
  constructor(
    ...
    private readonly subscribesRepository: Repository<SubscriptionEntity>,
  ) {}

  async create(subscribeDto: SubscribeDto): Promise<SubscriptionEntity> {
    const subscribe: SubscriptionEntity =
      await this.subscribesRepository.create(subscribeDto);
  ...
  async getAll(): Promise<SubscriptionEntity[]> {
    const subscribes: SubscriptionEntity[] =
      await this.subscribesRepository.find();
    return subscribes;
  }

  async getById(id: number): Promise<SubscriptionEntity> {
    const subscribe: SubscriptionEntity =
      await this.subscribesRepository.findOne({ where: { id: id } });
    return subscribe;
  }
