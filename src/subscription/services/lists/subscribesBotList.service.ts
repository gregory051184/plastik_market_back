...
export class SubscribesBotListService implements OnModuleInit {
        ...
        try {
            ...
            if (subscribes.length > 0) {
                const cit = await subscribes.map((subscribe: any) =>
                    [{text: `${subscribe.title}`, callback_data: `subscribe_${subscribe.id}`}],
                )
        ...
        try {
            ...
            if (currentUser.admin) {
                ...
                if (subscribes.length > 0) {
                    await bot.sendMessage(
                        chatId,
                        'Выберите подписку для изменения',
                        {
                            reply_markup: JSON.stringify({
                                inline_keyboard: sub
                            })
    ...
    async getAllSubscribeForDeleteList(bot: any, chatId: string): Promise<void> {
        ...
        try {
            ...
            const cit = await subscribes.map((subscribe: any) =>
                [{text: `${subscribe.title}`, callback_data: `delete_this_subscribe_${subscribe.id}`}]
            )
