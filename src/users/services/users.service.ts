...
export class UsersService {
    ...
    async getAllAdmins(): Promise<UserEntity[]> {
        const admins: UserEntity[] = await this.usersRepository.find({where: {admin: true}});
        return admins;
    };

    async getByFirstName(firstName: string): Promise<UserEntity[]> {
        const user: UserEntity[] = await this.usersRepository.find({where: {firstName: firstName}});
        return user;
    };

    async getByUsername(username: string): Promise<UserEntity[]> {
        const user: UserEntity[] = await this.usersRepository.find({where: {username: username}});
        return user;
    };
