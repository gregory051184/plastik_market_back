...
export class UsersBotListsService implements OnModuleInit {
    ...
    try {
      ...
      if (currentUser.admin) {
        ...
        const listOfAdmins = await admins.map((admin: any) => [
          {
            text: `Подробнее о ${admin.username}`,
            callback_data: `user_${admin.id}`,
          },
        ]);
        ...
        await bot
          ...
          .then(async () => {
            await bot.once('message', async (message: any) => {
              const users: any = await this.usersService.getByFirstName(
                this.commonService.messageText(message),
              );
              if (users.length > 0) {
                const listOfUsers = await users.map((user: any) => [
                  {
                    text: `Подробнее о ${user.username}`,
                    callback_data: `user_${user.id}`,
                  },
                ]);
                await bot.sendMessage(
                  chatId,
                  `СПИСОК ПОЛЬЗОВАТЕЛЕЙ С ИМЕНЕМ ${this.commonService.messageText(
                    message,
                  )}`,
                  {
                    reply_markup: JSON.stringify({
                ...
                await bot.sendMessage(
                  chatId,
                  `Пользователей с именем ${this.commonService.messageText(
                    message,
                  )} не найдено`,
                  usersAdminMenuKeyboard,
                );
  ...
  async getUserByUserNameList(bot: any, chatId: string) {
    try {
      ...
      if (currentUser.admin) {
        await bot
          .sendMessage(chatId, 'Введите имя пользователя (username)')
          .then(async () => {
            await bot.once('message', async (message: any) => {
              const users: any = await this.usersService.getByUsername(
                this.commonService.messageText(message),
              );
              if (users.length > 0) {
                const listOfUsers = await users.map((user: any) => [
                ...
                await bot.sendMessage(
                  chatId,
                  `СПИСОК ПОЛЬЗОВАТЕЛЕЙ С ИМЕНЕМ ПОЛЬЗОВАТЕЛЯ (USERNAME) ${this.commonService.messageText(
                    message,
                  )}`,
                  {
                    reply_markup: JSON.stringify({
                ...
                await bot.sendMessage(
                  chatId,
                  `Пользователей с именем пользователя ${this.commonService.messageText(
                    message,
                  )} не найдено`,
                  usersAdminMenuKeyboard,
                );
