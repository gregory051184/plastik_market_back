...
import {
  advertisementAdminMenuKeyboard,
  AdvertisementEntity,
  urls,
} from '@app/common';
...
export class AdvertisementsBotListService implements OnModuleInit {
  ...
  async getAllAdvertisementsList(
    bot: any,
    chatId: string,
    itemId?: number,
  ): Promise<void> {
  ...
  async getAllAdvertisementForUpdateList(
    bot: any,
    chatId: string,
  ): Promise<void> {
    try {
      ...
      const adverts = await advertisements.map((advertisement: any) => [
        {
          ...
          web_app: {
            url: `${urls.update_advertisement_form}/` + `${advertisement.id}`,
          },
        },
      ]);
  ...
  async getAllAdvertisementForDeleteList(
    bot: any,
    chatId: string,
  ): Promise<void> {
    try {
      ...
      const adverts = await advertisements.map((advertisement: any) => [
        {
          text: `${advertisement.title}`,
          callback_data: `delete_this_advertisement_${advertisement.id}`,
        },
      ]);

      if (advertisements.length > 0) {
        await bot.sendMessage(chatId, 'Выберите рекламный блок для удаления', {
          reply_markup: JSON.stringify({
            inline_keyboard: adverts,
          }),
        });
