...
export class AdvertisementService {
  ...
  async create(
    ...
    const advertisement: AdvertisementEntity =
      await this.advertisementsRepository.create(advertisementDto);
    return advertisement;
  }

  async update(advertisementUpdateDto: AdvertisementUpdateDto): Promise<void> {
    await this.advertisementsRepository.update(advertisementUpdateDto.id, {
  ...
  async findAll(): Promise<AdvertisementEntity[]> {
    const advertisements: AdvertisementEntity[] =
      await this.advertisementsRepository.find();
    return advertisements;
  }

  async findById(id: number): Promise<AdvertisementEntity> {
    const advertisement: AdvertisementEntity =
      await this.advertisementsRepository.findOne({ where: { id: id } });
    return advertisement;
  }
