    ...
    async getItemByIdItem(bot: any, chatId: string, id: number): Promise<void> {
        try {
                        ...
                        `,
                            ...
                            reply_markup: JSON.stringify({
                                inline_keyboard: [
                                    ...
                                    [
                                        {
                                            text: item.owner !== chatId.toString() ? 'Усилить объявление' : '',
                                            callback_data: `to_advertisements_list_${item.id}`
                                        }
                                    ],
                                    /*[
                                        {
                                            text: item.forSale && !item.sold ? 'Пометить как проданный' : 'Пометить как купленный',
                                    ...
                                    [
                                        {
                                            text: !item.sold ? 'Пометить ка завершённый' : '',
                                            callback_data: `mark_item_as_sold_${item.id}`
                                        }
                                    ...
                                    [
                                        {
                                            text: !cart.items.find((it: any) => it.id === item.id) && item.owner !== chatId.toString() ? adminButtons.carts.addToFavorites : '',
            ...
            Описание - ${item.description}
                                    ...
                                    [
                                        {
                                            ...
                                            web_app: {url: `${urls.update_item_form}/` + `${item.id}/` + `${chatId}`}
                                        }
                ...
                if (!currentUser.admin) {
                    await bot.sendMessage(chatId, `Возникла непредвиденная ошибка`, itemsUserMenuKeyboard)
                }
                if (currentUser.admin) {
                    await bot.sendMessage(chatId, `Возникла непредвиденная ошибка`, itemsAdminMenuKeyboard)
                }
            }
