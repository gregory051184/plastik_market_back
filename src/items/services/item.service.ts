...
export class ItemService {
  ...
  async create(file: any, itemDto: ItemDto): Promise<ItemEntity> {
    ...
    const fileName: string =
      `${itemDto.owner}_` + `${itemDto.title}_` + `${file[0].originalname}`;
    const existingItem = await this.itemRepository.findOne({
      where: { image: path.join(path.resolve(fileName)) },
    });
    //const filePath = path.resolve(__dirname, '..', '..', `itemsMainImages/${itemDto.owner}`);
  ...
  async update(itemUpdateDto: ItemUpdateDto, file?: any): Promise<void> {
    if (file) {
      ...
      const fileName: string =
        `${itemUpdateDto.owner}_` +
        `${itemUpdateDto.title}_` +
        `${file[0].originalname}`;
      ...
      fs.writeFile(path.join(path.resolve(fileName)), file[0].buffer, (err) => {
        // записываем файлы на сервер
        if (err) console.log('Не удалось создать файл');
      });
