...
export class CategoriesBotListsService implements OnModuleInit {
    ...
    async getAllCategoriesList(bot: any, chatId: string, forSale: boolean): Promise<void> {
        try {
            ...
            if (categories.length > 0) {
                if (!forSale) {
                    const cats = await categories.map((category: any) =>
                        [{text: `${category.title}`, callback_data: `category_for_buying_${category.id}`}]
                    )
                ...
                if (forSale) {
                    const cats = await categories.map((category: any) =>
                        [{text: `${category.title}`, callback_data: `category_for_sale_${category.id}`}]
                    )
                    await bot.sendMessage(
                        chatId,
                        'Выберите категорию для продажи',
                        {
                            reply_markup: JSON.stringify({
                                inline_keyboard: cats
                            })
    ...
    async getAllCategoryForDeleteList(bot: any, chatId: string): Promise<void> {
        ...
        try {
            ...
            const cats = await categories.map((category: any) =>
                [{text: `${category.title}`, callback_data: `delete_this_category_${category.id}`}]
            )
